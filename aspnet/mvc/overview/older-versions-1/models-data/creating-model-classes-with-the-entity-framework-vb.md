...
\_db 필드는 영화 데이터베이스 테이블에서 레코드를 검색에 index () 작업 내에서 사용 합니다. 식 \_db입니다. MovieSet 영화 데이터베이스 테이블에서 모든 레코드를 나타냅니다. 영화 집합이 Movie 개체의 제네릭 컬렉션을 변환할 tolist () 메서드 사용 됩니다. 목록 (동영상)입니다.
