...
저장 단추 (플로피 디스크 아이콘)를 클릭 하 여 수정한 후에 엔터티 데이터 모델을 저장 해야 합니다. Entity Designer는 내부적으로 Visual Basic .NET 클래스 집합을 생성 합니다. 솔루션 탐색기 창에서 MoviesDBModel 파일을 열어 이러한 클래스를 볼 수 있습니다.

다음에 Entity Designer를 사용할 때 변경 내용을 덮어쓸 수 있으므로 디자이너 .vb 파일의 코드를 수정 하지 마세요. 디자이너 .vb 파일에 정의 된 엔터티 클래스의 기능을 확장 하려는 경우에는 개별 파일에 *partial 클래스* 를 만들 수 있습니다.
...
\_db 필드는 Index () 작업 내에서 영화 데이터베이스 테이블의 레코드를 검색 하는 데 사용 됩니다. 식 \_db입니다. MovieSet는 동영상 데이터베이스 테이블의 모든 레코드를 나타냅니다. ToList () 메서드를 사용 하 여 영화 집합을 동영상 개체의 제네릭 컬렉션인 목록 (동영상)으로 변환할 수 있습니다.
...
목록 2의 뷰에는 각 영화 레코드를 반복 하 고 영화 레코드의 제목 및 감독 속성 값을 표시 하는 **For each** 루프가 포함 되어 있습니다. 그러면 각 레코드 옆에 편집 및 삭제 링크가 표시 됩니다. 또한 보기의 아래쪽에 영화 추가 링크가 표시 됩니다 (그림 7 참조).

**그림 7-인덱스 뷰**

![clip_image014](creating-model-classes-with-the-entity-framework-vb/_static/image7.jpg)

인덱스 뷰는 형식화 된 *뷰입니다*. 인덱스 뷰에 Inherits 특성이 포함 된 &lt;% @ Page%&gt; 지시어가 있습니다. Inherits 특성은 ViewData 속성을 동영상 개체의 강력한 형식의 제네릭 목록 컬렉션 (영화 목록)으로 캐스팅 합니다.
...
**목록 3 – Controllers\HomeController.vb (메서드 추가)**
...
**목록 4 – Controllers\HomeController.vb (Edit 메서드)**
...
**목록 6--\Controllers\HomeController.vb (Delete 동작)**
