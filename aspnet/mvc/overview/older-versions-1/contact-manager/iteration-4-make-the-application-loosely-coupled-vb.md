...
먼저 모든 수행 해야 하는 데이터 액세스 메서드를 설명 하는 인터페이스를 만드는 해야 합니다. IContactManagerRepository 인터페이스 목록 1에 포함 됩니다. 이 인터페이스에는 다섯 가지 방법을 설명합니다. CreateContact(), DeleteContact(), EditContact(), GetContact, and ListContacts().
