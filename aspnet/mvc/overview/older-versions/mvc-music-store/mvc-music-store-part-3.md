...
- 변경 내용을 검토해 보겠습니다. HomeController의 Index 작업 메서드에 찾아서 코드 보기 템플릿은 표준 명명 규칙을 따른 때문에 "반환 View()"를 호출 하는 경우에 \Views\Home\Index.cshtmlView 템플릿을 표시 합니다.
...
*참고: 궁금한 경우는 {get; 설정;} 표기법 이루어지는 사용 C#의 자동 구현 속성 기능입니다. 지원 필드를 선언 하기를 요구 하지 않고 속성의 이점을 제공 합니다.*
...
*참고: 잘 모르는 경우 C#에 album 변수는 런타임에 바인딩된 의미 var을 사용 하 고 가정할 수도 있습니다. C# 컴파일러를 사용 하 여 형식 유추 기반으로 새로운는 앨범 형식의 해당 앨범을 인지 합니다. 변수를 할당 하 고 컴파일 시간 검사 및 Visual Studio 코드 편집기를 얻게 되므로 로컬 앨범 변수의 앨범 유형으로 컴파일하는 –는 올바르지 않습니다. 이 옵션을 지원 합니다.*
...
*참고: 이 경우 현재 보기를 렌더링 하는 동일한 컨트롤러 내의 다른 작업에만 연결 하는 것 때문에 컨트롤러의 이름을 지정 해야 하지 않았습니다.*
