...
> 
> - **OnActionExecuting**(ActionExecutingContext filterContext): 메서드는 작업 바로 전에 호출 됩니다.
> - **OnActionExecuted**(ActionExecutedContext filterContext): 동작 메서드가 호출 된 후 및 하기 전에 결과 (이전 보기 렌더링) 실행 됩니다.
> - **OnResultExecuting**(ResultExecutingContext filterContext): 결과 직전 (이전 보기 렌더링) 실행 됩니다.
...
#### <a name="task-2-injecting-a-new-code-interceptor-into-the-storecontroller-class"></a>작업 2: 새 코드 인터셉터를 StoreController 클래스에 삽입
...
3. 이동할 **/ActionLog** 로그 보기에 대 한 초기 상태를 확인 합니다.
    ...
    *페이지 작업 하기 전에 로그 추적기 상태*
4. 중 하나를 클릭 합니다 **장르** 메뉴에서 사용할 수 있는 앨범을 검색 하는 등 일부 작업을 수행 합니다.
5. 이 이번에 따라 방문 된 추적 되는 필터의 순서 값으로 정렬 된 것을 확인 합니다. **CustomActionFilter** 로그의 첫 번째입니다.
