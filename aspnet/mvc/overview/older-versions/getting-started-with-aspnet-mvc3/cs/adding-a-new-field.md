...
`Model` 클래스를 업데이트 했으므로 이제 새 `Rating` 속성을 지원 하기 위해 *\Views\Movies\Index.cshtml* 및 *\Views\Movies\Create.cshtml* 뷰 템플릿을 업데이트 해야 합니다.
...
클래스 이름을 "MovieInitializer"로 합니다. 다음 코드를 포함 하도록 `MovieInitializer` 클래스를 업데이트 합니다.
...
파일의 맨 위에 두 개의 using 문을 추가 해 보겠습니다. 첫 번째는 Entity Framework 네임 스페이스를 참조 하 고, 두 번째는 `MovieInitializer` 클래스가 있는 네임 스페이스를 참조 합니다.

[!code-csharp[Main](adding-a-new-field/samples/sample6.cs)]

그런 다음 `Application_Start` 메서드를 찾고 아래와 같이 메서드 시작 부분에 `Database.SetInitializer`에 대 한 호출을 추가 합니다.
