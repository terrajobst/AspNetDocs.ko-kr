...
1. 응용 프로그램 Web.config 파일의 system.web 요소에는 customErrors 요소 있으면 모드 특성을 "off"로 변경 합니다. 그렇지 않은 경우 customErrors 요소를 요소에 추가 system.web 모드 특성을 "off"로 설정 하 여 다음 예와에서 같이: 
...
액세스가 거부되었습니다. (예외가 발생한 HRESULT: 0x80070005 (E\_ACCESSDENIED))
...
프로젝트 파일에서 PreSource 요소를 편집 하는 방법에 대 한 정보를 참조 하세요. [방법: 프로젝트 파일에서 배포 설정을 편집](https://msdn.microsoft.com/library/ff398069(v=vs.100).aspx)합니다. 사용자 또는 역할을 개발 데이터베이스에서 대상 데이터베이스에 필요한 경우 지원에 대 한 호스팅 공급자에 게 문의 합니다.
...
오류: 스트림 데이터를 ' sitemanifest/dbFullSql [@path= 'C:\TEMP\AdventureWorksGrant.sql']/sqlScript'를 사용할 수 없습니다.
