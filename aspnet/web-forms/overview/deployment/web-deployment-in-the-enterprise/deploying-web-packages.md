...
사용 하 여 대 한 자세한 내용은 합니다 *. deploy.cmd* 내용은 웹 패키지를 배포 하는 파일 [방법: Deploy.cmd 파일을 사용 하 여 배포 패키지를 설치](https://msdn.microsoft.com/library/ff356104.aspx)합니다.
...
웹 패키지를 실행 하 여 배포 되는 여부에 관계 없이 합니다 *. deploy.cmd* 파일 또는 MSDeploy.exe를 직접 사용 하 여 인증 형식을 지정 해야 합니다. 웹 배포는 두 개의 가능한 값을 허용합니다. **NTLM** 나 **기본**입니다. 기본 인증을 지정 하는 경우 또한 사용자 이름 및 암호를 제공 해야 합니다. 가지 인증 유형을 선택 하는 경우 주의 해야 하는 다양 한 요인이 있습니다.
