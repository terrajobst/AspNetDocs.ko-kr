...
## <a name="step-1-call-the-registerrequirescontrolstate-method"></a>1단계: RegisterRequiresControlState 메서드를 호출 합니다.
...
## <a name="step-2-override-savecontrolstate"></a>2단계: SaveControlState를 재정의 합니다.

SaveControlState 메서드 마지막 포스트백 이후에 컨트롤 컨트롤에 대 한 상태 변경 내용을 저장합니다. 컨트롤의 상태를 나타내는 개체를 반환 합니다.

## <a name="step-3-override-loadcontrolstate"></a>3단계: LoadControlState 재정의
...
**중요 한**: MSDN 설명서 (및 설명서를 다른 곳에서)이이 값 (킬로바이트) (바이트)에서 이며 기본값은 256 있는지를 지정 합니다. (킬로바이트)에서 실제로 지정 된 값 및 기본값은 80입니다. 기본값은 80 K 함으로써 대형 개체 힙에 버퍼 종료 하지 않습니다 확인 합니다.
...
| **모드** | **설명** |
| --- | --- |
| 변환 | 태그를 요청 하는 브라우저의 프로토콜에 따라 변환 됩니다. |
