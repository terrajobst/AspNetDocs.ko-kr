...
- Web Forms에서 jQuery 사용 하 여 비간섭 클라이언트 쪽 유효성 검사를 활용
- 세분화 된 요청 유효성 검사 구현
...
1. [연습 1: ASP.NET Web Forms에서 모델 바인딩](#Exercise1)
...
### <a name="exercise-1-model-binding-in-aspnet-web-forms"></a>연습 1: ASP.NET Web Forms에서 모델 바인딩
...
9. ItemTemplate 콘텐츠를 다음 코드로 대체 합니다.
    ...
    ItemType 속성을 설정 하면 데이터 바인딩 식의 범위에 생성 될 두 개의 새 형식화 된 변수: **항목** 하 고 **binditem을**입니다. 데이터 바인딩 식에서 이러한 강력한 형식의 변수를 사용할 수 있으며 Visual Studio 개발 환경의 모든 혜택을 얻을 수 있습니다.
...
4. 오픈 **CustomerDetails.aspx** 코드 숨김 및 SaveCustomer 메서드를 찾습니다. 이 메서드는 새 고객 삽입 될 때 호출 됩니다 및 FormView 컨트롤 값에서 고객 매개 변수를 받습니다. 페이지 간의 매핑을 제어 하 고 ASP.NET는 모든 데이터 주석 특성에 대 한 모델 유효성 검사를 실행 하 고 있는 경우 발생 하는 오류와 함께 ModelState 사전 입력 매개 변수 개체가 발생 경우.
...
1. 로 이동 합니다 [Azure 관리 포털](https://manage.windowsazure.com/) 구독과 연결 된 Microsoft 자격 증명을 사용 하 여 로그인 합니다.
    ...
    > [!NOTE]
    > Azure를 사용 하 여 10 개의 ASP.NET 웹 사이트를 무료로 호스트할 수 있으며 다음 트래픽 증가 따라 확장할 수 있습니다. 등록할 수 있습니다 [여기](https://aka.ms/aspnet-hol-azure)합니다.
