...
## <a name="step-1-adding-a-new-aspnet-page"></a>1단계: 새 ASP.NET 페이지 추가
...
## <a name="step-2-examining-the-renderedidchanges"></a>2단계: 렌더링 된 검사`ID`변경
...
**그림 04**: 렌더링 된 `id` 특성에 기반을 `ID` 명명 컨테이너의 값
...
## <a name="step-3-programmatically-referencing-web-controls-viafindcontrol"></a>3단계: 통해 웹 컨트롤을 프로그래밍 방식으로 참조`FindControl`
...
## <a name="step-4-using-the-correctidattribute-value-in-client-side-script"></a>4단계: 사용 하 여 올바른`id`클라이언트 쪽 스크립트에서 값을 특성
...
- [방법: 참조 ASP.NET 마스터 페이지 콘텐츠](https://msdn.microsoft.com/library/xxwa0ff0.aspx)
