...
> [!NOTE]
> 이전 자습서에서 설명 했 듯이 것은 매우 중요 GridView가의 뷰 상태 수 (기본 동작)을 사용할 수 있도록 합니다. GridView가 설정 하는 경우 `EnableViewState` 속성을 `false`, 동시 사용자가 실수로 삭제 하거나 편집할 레코드의 위험이 있습니다. 참조 [경고: 동시성 문제 사용 하 여 ASP.NET 2.0 Gridview/DetailsView/FormViews 편집 하는 지원 및/또는 삭제 하 고 있는 뷰 상태가 사용 불가능](http://scottonwriting.net/sowblog/archive/2006/10/03/163215.aspx) 자세한 내용은 합니다.
