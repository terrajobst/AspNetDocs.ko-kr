...
## <a name="step-1-adding-a-computed-column-to-thesupplierstable"></a>1단계: 계산된 열을 추가 합니다`Suppliers`테이블
...
SQL에서 문자열을 연결 수는 참고를 사용 하 여는 `+` 연산자입니다. `CASE` 문은 기존의 프로그래밍 언어의 조건부와 같은 사용할 수 있습니다. 위 식에서의 `CASE` 로 문을 읽을 수 있습니다. 하는 경우 `ContactTitle` 아닙니다 `NULL` 출력 한 다음는 `ContactTitle` 그렇지 쉼표를 사용 하 여 연결 하는 값 emit 아무 작업도 수행 합니다. 유용성에 대 한 자세한 내용은 합니다 `CASE` 문을 참조 [SQL의 Power `CASE` 문을](http://www.4guysfromrolla.com/webtech/102704-1.shtml)합니다.
...
[![ASuppliers 테이블에 열 이름이 FullContactName 계산을 dd](working-with-computed-columns-vb/_static/image2.png)](working-with-computed-columns-vb/_static/image1.png)

**그림 1**: 계산 된 열 이라는 추가 `FullContactName` 에 `Suppliers` 테이블 ([클릭 하 여 큰 이미지 보기](working-with-computed-columns-vb/_static/image3.png))
...
Microsoft SQL Server의 계산된 열에 대 한 자세한 내용은 참조는 [기술 문서](https://msdn.microsoft.com/library/ms191250.aspx)합니다. 또한 체크 아웃을 [방법: 계산 열 지정](https://msdn.microsoft.com/library/ms188300.aspx) 계산된 열 만들기의 단계별 연습에 대 한 합니다.
...
## <a name="step-2-viewing-the-computed-column-s-values"></a>2단계: 계산된 열의 값 보기
...
[![TFullContactName 사용 (ContactTitle, CompanyName) 형식 ContactName](working-with-computed-columns-vb/_static/image5.png)](working-with-computed-columns-vb/_static/image4.png)

**그림 2**: 합니다 `FullContactName` 형식을 사용 `ContactName` (`ContactTitle`를 `CompanyName`) ([클릭 하 여 큰 이미지 보기](working-with-computed-columns-vb/_static/image6.png))


## <a name="step-3-adding-thesupplierstableadapterto-the-data-access-layer"></a>3단계: 추가 된`SuppliersTableAdapter`데이터 액세스 계층
...
[![C새 저장 프로시저 옵션을 설치할 만들기](working-with-computed-columns-vb/_static/image8.png)](working-with-computed-columns-vb/_static/image7.png)

**그림 3**: 새 저장된 프로시저 만들기 옵션을 선택 ([클릭 하 여 큰 이미지 보기](working-with-computed-columns-vb/_static/image9.png))
...
[![Customize Auto-Generated 저장 프로시저의 이름](working-with-computed-columns-vb/_static/image11.png)](working-with-computed-columns-vb/_static/image10.png)

**그림 4**: Auto-Generated 저장 프로시저의 이름을 사용자 지정 ([클릭 하 여 큰 이미지 보기](working-with-computed-columns-vb/_static/image12.png))
...
[![RGetData 메서드 GetSuppliers ename](working-with-computed-columns-vb/_static/image14.png)](working-with-computed-columns-vb/_static/image13.png)
...
## <a name="step-4-including-the-computed-column-in-the-tableadapter-s-main-query"></a>4단계: 계산된 열을 포함 하 여 TableAdapter가 기본 쿼리에
...
[![R취소 DataTable의 열을 업데이트 하려면 TableAdapter의 구성 마법사](working-with-computed-columns-vb/_static/image17.png)](working-with-computed-columns-vb/_static/image16.png)

**그림 6**: Tableadapter의 DataTable의 열을 업데이트 하려면 구성 마법사를 실행 합니다. ([클릭 하 여 큰 이미지 보기](working-with-computed-columns-vb/_static/image18.png))
...
[![T읽기 전용으로 표시 되어 그 FullContactName 열](working-with-computed-columns-vb/_static/image20.png)](working-with-computed-columns-vb/_static/image19.png)

**그림 7**: 합니다 `FullContactName` 열은 읽기 전용으로 표시 됩니다 ([큰 이미지를 보려면 클릭](working-with-computed-columns-vb/_static/image21.png))


## <a name="step-5-adding-agetsupplierbysupplieridmethod-to-the-tableadapter"></a>5단계: 추가 된`GetSupplierBySupplierID`TableAdapter 메서드
...
[![C설치할 옵션 행을 반환 하는 SELECT](working-with-computed-columns-vb/_static/image23.png)](working-with-computed-columns-vb/_static/image22.png)

**그림 8**: 옵션 행을 반환 하는 선택 ([클릭 하 여 큰 이미지 보기](working-with-computed-columns-vb/_static/image24.png))
...
[![N저장 프로시저 Suppliers_SelectBySupplierID \ 이름](working-with-computed-columns-vb/_static/image26.png)](working-with-computed-columns-vb/_static/image25.png)

**그림 9**: 저장 프로시저의 이름을 `Suppliers_SelectBySupplierID` ([큰 이미지를 보려면 클릭](working-with-computed-columns-vb/_static/image27.png))
...
[![Name TableAdapter 메서드 FillBySupplierID 및 GetSupplierBySupplierID](working-with-computed-columns-vb/_static/image29.png)](working-with-computed-columns-vb/_static/image28.png)
...
## <a name="step-6-creating-the-business-logic-layer"></a>6단계: 비즈니스 논리 레이어 만들기
...
## <a name="step-7-working-with-the-computed-column-from-the-presentation-layer"></a>7단계: 프레젠테이션 계층에서 계산된 열 작업
...
[![Configure SuppliersBLLWithSprocs 클래스를 사용 하는 ObjectDataSource](working-with-computed-columns-vb/_static/image32.png)](working-with-computed-columns-vb/_static/image31.png)

**그림 11**: ObjectDataSource를 사용 하 여 구성 합니다 `SuppliersBLLWithSprocs` 클래스 ([큰 이미지를 보려면 클릭](working-with-computed-columns-vb/_static/image33.png))
...
[![E공급 업체는 표에 나열 된 대 한 ach](working-with-computed-columns-vb/_static/image35.png)](working-with-computed-columns-vb/_static/image34.png)

**그림 12**: 각 공급 업체는 표에 나열 된 ([클릭 하 여 큰 이미지 보기](working-with-computed-columns-vb/_static/image36.png))
...
[![TFullContactName 열 담당 하 고 편집할 수 없습니다](working-with-computed-columns-vb/_static/image38.png)](working-with-computed-columns-vb/_static/image37.png)

**그림 13**: 합니다 `FullContactName` 열이 편집할 수 없습니다 ([큰 이미지를 보려면 클릭](working-with-computed-columns-vb/_static/image39.png))
