...
F5 키를 눌러 애플리케이션을 빌드하고 실행합니다. 응용 프로그램을 처음 실행 하면 로그인 화면을 표시 합니다.
...
- `TodoController`: 할 일 항목에 대 한 HTTP 요청을 처리
...
| GET /api/todo/*id* | `GetTodoList` | ID 별로 할 일 목록 가져오기 |
...
모델 클래스의 속성 중 상당수는 "ko.observable" 유형입니다. 관찰 가능 개체는 Knockout 있을까요는 핵심입니다. [Knockout 설명서](http://knockoutjs.com/documentation/introduction.html): 관찰 가능 개체는 "JavaScript 개체 변경 내용에 대 한 구독자에 알릴 수 있는." 관찰 가능 개체의 값이 변경 되 면 Knockout 해당 관찰 가능 개체에 바인딩된 모든 HTML 요소를 업데이트 합니다. 예를 들어 todoItem 제목과 수행 속성에 대 한 관찰 가능 개체에 있습니다.
...
- **visible**: 표시 설정/해제 하는 데 사용 합니다. 오류 메시지를 표시 되도록 하거나 컬렉션이 비어 있을 때 태그를 숨깁니다.
- **value**: 양식 값을 채우는 데 사용 합니다.
- **click**: 클릭 이벤트를 뷰 모델에서 함수에 바인딩합니다.
