...
> 
> <span data-ttu-id="a27a6-243">이 호스팅 공급자가 서버의 전역 어셈블리 캐시 (GAC)에 ASP.NET 웹 페이지 베타 1 어셈블리를 설치 하는 경우 발생할 수 있습니다.</span><span class="sxs-lookup"><span data-stu-id="a27a6-243">This can happen if the hosting provider has installed the ASP.NET Web Pages Beta 1 assemblies into the server's global application cache (GAC).</span></span> <span data-ttu-id="a27a6-244">GAC의 어셈블리 우선적으로 로컬에 설치 된 어셈블리를 *Bin* 폴더입니다.</span><span class="sxs-lookup"><span data-stu-id="a27a6-244">Assemblies in the GAC get precedence over assemblies installed locally in the *Bin* folder.</span></span>
> 
...
>     - <span data-ttu-id="a27a6-293">Copy *C:\Program Files\Microsoft SQL Server Compact Edition\v4.0\Private\x86\\*\* **to** *\Bin\x86*</span><span class="sxs-lookup"><span data-stu-id="a27a6-293">Copy *C:\Program Files\Microsoft SQL Server Compact Edition\v4.0\Private\x86\\*\* **to** *\Bin\x86*</span></span>
...
> 
> <span data-ttu-id="a27a6-357">**해결 방법** 다음 해결 방법 중 하나를 사용 하 여 (해결 방법 2 필요한 Microsoft Windows Vista 이상):</span><span class="sxs-lookup"><span data-stu-id="a27a6-357">**Workaround** Use any one of the following workarounds (workaround #2 requires Microsoft Windows Vista or higher):</span></span>
> 
> 
> 1. <span data-ttu-id="a27a6-358">복사 합니다 *Microsoft.Web.dll* 하 고 *Microsoft.Web.Administration.dll* WebMatrix 설치 위치에서 어셈블리를 *bin* wcf 디렉터리 응용 프로그램입니다.</span><span class="sxs-lookup"><span data-stu-id="a27a6-358">Copy the *Microsoft.Web.dll* and *Microsoft.Web.Administration.dll* assemblies from the WebMatrix installation location to the *bin* directory of the WCF application.</span></span> <span data-ttu-id="a27a6-359">기본적으로 WebMatrix에서 설치 합니다 *Microsoft WebMatrix* 시스템의 하위 폴더 *Program Files* 폴더입니다.</span><span class="sxs-lookup"><span data-stu-id="a27a6-359">By default, WebMatrix is installed in the *Microsoft WebMatrix* subfolder under the system's *Program Files* folder.</span></span>
> 2. <span data-ttu-id="a27a6-360">Microsoft Windows Vista 이상에서의 어셈블리에 symlink를 만듭니다는 *bin* 디렉터리에서 다음 명령을 사용 합니다.</span><span class="sxs-lookup"><span data-stu-id="a27a6-360">On Microsoft Windows Vista or higher, create a symlink to the assemblies in the *bin* directory using the following commands.</span></span> <span data-ttu-id="a27a6-361">(이 방법은 이점이 어셈블리의 복사본이 만들어지지 않습니다.)</span><span class="sxs-lookup"><span data-stu-id="a27a6-361">(This approach has the advantage that it does not create a copy of the assemblies.)</span></span>
> 
>     [!code-console[Main](beta3/samples/sample18.cmd)]
> 3. <span data-ttu-id="a27a6-362">두 어셈블리를 GAC에 설치 합니다.</span><span class="sxs-lookup"><span data-stu-id="a27a6-362">Install the two assemblies in the GAC.</span></span> <span data-ttu-id="a27a6-363">상승된 된 프롬프트에서 다음 명령을 실행 합니다.</span><span class="sxs-lookup"><span data-stu-id="a27a6-363">From an elevated prompt, run the following commands:</span></span>
> 
>     [!code-console[Main](beta3/samples/sample19.cmd)]


#### <a name="issue-webmatrix-beta-3-is-unable-to-perform-certain-tasks-that-require-elevation"></a><span data-ttu-id="a27a6-364">문제: WebMatrix 베타 3은 권한 상승 해야 하는 특정 작업을 수행할 수 없습니다.</span><span class="sxs-lookup"><span data-stu-id="a27a6-364">Issue: WebMatrix Beta 3 is unable to perform certain tasks that require elevation</span></span>

> <span data-ttu-id="a27a6-365">WebMatrix 베타 3가 다음과 같은 상황에서 추가 구성 요소를 설치 하는 등의 권한 상승 해야 하는 특정 작업을 수행할 수 없습니다.</span><span class="sxs-lookup"><span data-stu-id="a27a6-365">WebMatrix Beta 3 is unable to perform certain tasks that require elevation, such as installing additional components in the following situations:</span></span>
> 
> - <span data-ttu-id="a27a6-366">Windows Vista 또는 Windows 7에서 관리자 권한이 없는 계정으로 로그온 한 사용자 계정 컨트롤 (UAC)을 사용할 수 없습니다.</span><span class="sxs-lookup"><span data-stu-id="a27a6-366">On Windows Vista or Windows 7, you are logged in with an account that does not have administrative privileges and User Account Control (UAC) is disabled.</span></span>
> - <span data-ttu-id="a27a6-367">Microsoft Windows XP 또는 Microsoft Windows Server 2003을 사용 하는 있습니다.</span><span class="sxs-lookup"><span data-stu-id="a27a6-367">You are using Microsoft Windows XP or Microsoft Windows Server 2003.</span></span>
> 
> <span data-ttu-id="a27a6-368">**해결 방법**</span><span class="sxs-lookup"><span data-stu-id="a27a6-368">**Workaround**</span></span>  
> <span data-ttu-id="a27a6-369">WebMatrix Beta 3에서 대부분의 작업에는 관리 권한이 필요 하지 않습니다.</span><span class="sxs-lookup"><span data-stu-id="a27a6-369">Most tasks in WebMatrix Beta 3 do not require administrative permission.</span></span> <span data-ttu-id="a27a6-370">이러한 조작자에 대 한 다음이 단계를 수행 또는 관리자로 서 작업을 수행할 수 있습니다.</span><span class="sxs-lookup"><span data-stu-id="a27a6-370">For those that do, you can perform the operation as an administrator, or follow these steps:</span></span>
> 
> - <span data-ttu-id="a27a6-371">Windows Vista 또는 Windows 7에서 UAC를 사용 하도록 설정 합니다.</span><span class="sxs-lookup"><span data-stu-id="a27a6-371">On Windows Vista or Windows 7, enable UAC.</span></span>
> - <span data-ttu-id="a27a6-372">Windows XP에서 Administrators 보안 그룹에 사용자를 추가 합니다.</span><span class="sxs-lookup"><span data-stu-id="a27a6-372">On Windows XP, add the user to the Administrators security group.</span></span>


#### <a name="issue-site-from-web-gallery-is-disabled"></a><span data-ttu-id="a27a6-373">문제: "웹 갤러리에서 사이트"를 사용 하지 않도록 설정</span><span class="sxs-lookup"><span data-stu-id="a27a6-373">Issue: "Site from Web Gallery" is disabled</span></span>

> <span data-ttu-id="a27a6-374">합니다 **웹 갤러리에서 사이트** 웹 플랫폼 설치 관리자 3.0이 설치 되지 않은 경우 옵션은 사용할 수 없습니다.</span><span class="sxs-lookup"><span data-stu-id="a27a6-374">The **Site from Web Gallery** option is disabled if the Web Platform Installer 3.0 is not installed.</span></span>
> 
> <span data-ttu-id="a27a6-375">**해결 방법**</span><span class="sxs-lookup"><span data-stu-id="a27a6-375">**Workaround**</span></span>  
> <span data-ttu-id="a27a6-376">설치 합니다 [Microsoft 웹 플랫폼 설치 관리자 3.0](https://go.microsoft.com/fwlink/?LinkID=194638)합니다.</span><span class="sxs-lookup"><span data-stu-id="a27a6-376">Install the [Microsoft Web Platform Installer 3.0](https://go.microsoft.com/fwlink/?LinkID=194638).</span></span>


#### <a name="issue-on-windows-server-2003-iis-express-does-not-start-for-a-non-administrative-user"></a><span data-ttu-id="a27a6-377">문제: Windows Server 2003에서 IIS Express 시작 되지 않으면 관리자가 아닌 사용자에 대 한</span><span class="sxs-lookup"><span data-stu-id="a27a6-377">Issue: On Windows Server 2003, IIS Express does not start for a non-administrative user</span></span>

> <span data-ttu-id="a27a6-378">Windows Server 2003에서 페이지를 시작 하거나 IIS Express를 시작 하는 경우 IIS Express 시작 되지 않습니다.</span><span class="sxs-lookup"><span data-stu-id="a27a6-378">On Windows Server 2003, when you launch a page or start IIS Express, IIS Express does not start.</span></span> <span data-ttu-id="a27a6-379">웹 페이지 응용 프로그램 관리자가 아닌 사용자가 시작한 있는지 여부를 나타내는 오류가 표시 됩니다.</span><span class="sxs-lookup"><span data-stu-id="a27a6-379">For Web pages, an error is displayed that indicates that the application has been started by a non-administrative user.</span></span>
> 
> <span data-ttu-id="a27a6-380">**해결 방법**</span><span class="sxs-lookup"><span data-stu-id="a27a6-380">**Workaround**</span></span>  
> <span data-ttu-id="a27a6-381">관리자 권한으로 WebMatrix 베타 3을 시작 합니다.</span><span class="sxs-lookup"><span data-stu-id="a27a6-381">Start WebMatrix Beta 3 as an administrative user.</span></span> <span data-ttu-id="a27a6-382">자세한 내용은 다음 기술 자료 문서를 참조 하세요.</span><span class="sxs-lookup"><span data-stu-id="a27a6-382">For more details, see the following KnowledgeBase article:</span></span>  
>   
> [<span data-ttu-id="a27a6-383">관리자가 아닌 사용자가 시작 되는 응용 프로그램은 응용 프로그램이 Windows Vista, Windows Server 2003 또는 Windows XP에서 실행 되는 컴퓨터의 HTTP 트래픽을 수신할 수 없습니다.</span><span class="sxs-lookup"><span data-stu-id="a27a6-383">An application that is started by a non-administrative user cannot listen to the HTTP traffic of the computer on which the application is running in Windows Vista, Windows Server 2003, or Windows XP.</span></span>](https://support.microsoft.com/kb/939786)


#### <a name="issue-google-chrome-is-not-available-as-a-run-option"></a><span data-ttu-id="a27a6-384">문제: Google Chrome 실행 옵션으로 지원 되지 않습니다.</span><span class="sxs-lookup"><span data-stu-id="a27a6-384">Issue: Google Chrome is not available as a Run option</span></span>

> <span data-ttu-id="a27a6-385">Google Chrome 브라우저에서 목록에 표시 되지 않습니다 **실행할** 에 **홈** 탭 합니다.</span><span class="sxs-lookup"><span data-stu-id="a27a6-385">Google Chrome is not displayed in the list of browsers under **Run** on the **Home** tab.</span></span>
> 
> <span data-ttu-id="a27a6-386">**해결 방법**</span><span class="sxs-lookup"><span data-stu-id="a27a6-386">**Workaround**</span></span>  
> <span data-ttu-id="a27a6-387">Google Chrome의 일부 버전에서는 등록 하지 자체 올바르게 Windows의 기본 프로그램 기능을 사용 합니다.</span><span class="sxs-lookup"><span data-stu-id="a27a6-387">Some versions of Google Chrome do not register themselves correctly with the Default Programs feature in Windows.</span></span> <span data-ttu-id="a27a6-388">대 안으로, Google Chrome을 시작 합니다 *사용자 지정 및 Google Chrome 컨트롤* 메뉴에서 클릭 *옵션*를 클릭 하 고 *확인 Google Chrome 기본 브라우저*.</span><span class="sxs-lookup"><span data-stu-id="a27a6-388">As a workaround, start Google Chrome, click the *Customize and control Google Chrome* menu, click *Options*, and then click *Make Google Chrome my default browser*.</span></span>


#### <a name="issue-the-foreign-key-dialog-box-doesnt-allow-entering-a-primary-key"></a><span data-ttu-id="a27a6-389">문제: "외래 키" 대화 상자에 기본 키 입력을 허용 하지 않습니다.</span><span class="sxs-lookup"><span data-stu-id="a27a6-389">Issue: The "Foreign Key" dialog box doesn't allow entering a primary key</span></span>

> <span data-ttu-id="a27a6-390">합니다 **외래 키** 대화 상자 기본 키 테이블에서 기본 키 이름을 입력할 수를 허용 하지 않습니다.</span><span class="sxs-lookup"><span data-stu-id="a27a6-390">The **Foreign Key** dialog box does not allow you to enter the primary key name from the primary key table.</span></span>
> 
> <span data-ttu-id="a27a6-391">**해결 방법**</span><span class="sxs-lookup"><span data-stu-id="a27a6-391">**Workaround**</span></span>  
> <span data-ttu-id="a27a6-392">이 의도적입니다.</span><span class="sxs-lookup"><span data-stu-id="a27a6-392">This is intentional.</span></span> <span data-ttu-id="a27a6-393">기본 키 테이블에서 기본 키의 이름을 입력할 필요가 없습니다.</span><span class="sxs-lookup"><span data-stu-id="a27a6-393">You do not need to enter the name of the primary key from the primary key table.</span></span>


#### <a name="issue-the-relationships-button-is-disabled"></a><span data-ttu-id="a27a6-394">문제: "관계" 단추를 사용 하지 않도록 설정</span><span class="sxs-lookup"><span data-stu-id="a27a6-394">Issue: The "Relationships" button is disabled</span></span>

> <span data-ttu-id="a27a6-395">**관계** 단추를 **테이블** 탭에 **데이터베이스** SQL Server Compact 데이터베이스에 대 한 작업 영역을 사용할 수 없습니다.</span><span class="sxs-lookup"><span data-stu-id="a27a6-395">The **Relationships** button under the **Table** tab in the **Databases** workspace is disabled for SQL Server Compact databases.</span></span>
> 
> <span data-ttu-id="a27a6-396">**해결 방법**</span><span class="sxs-lookup"><span data-stu-id="a27a6-396">**Workaround**</span></span>  
> <span data-ttu-id="a27a6-397">없음</span><span class="sxs-lookup"><span data-stu-id="a27a6-397">None.</span></span> <span data-ttu-id="a27a6-398">SQL Server Compact 테이블 간의 관계를 지원 하지 않습니다.</span><span class="sxs-lookup"><span data-stu-id="a27a6-398">SQL Server Compact does not support relationships between tables.</span></span>


#### <a name="issue-parameterized-sql-queries-throw-exceptions"></a><span data-ttu-id="a27a6-399">문제: 예외를 throw 하는 매개 변수가 있는 SQL 쿼리</span><span class="sxs-lookup"><span data-stu-id="a27a6-399">Issue: Parameterized SQL queries throw exceptions</span></span>

> <span data-ttu-id="a27a6-400">SQL Server Compact 4.0에서는 데이터 형식 같은 지정 하지 않으면 `SqlDbType` 또는 `DbType` 매개 변수가 있는 쿼리에서 매개 변수의 경우 쿼리를 실행할 때 예외가 throw 됩니다.</span><span class="sxs-lookup"><span data-stu-id="a27a6-400">In SQL Server Compact 4.0, if you do not specify a data type such as `SqlDbType` or `DbType` for parameters in parameterized queries, an exception is thrown when the query runs.</span></span>
> 
> <span data-ttu-id="a27a6-401">**해결 방법**</span><span class="sxs-lookup"><span data-stu-id="a27a6-401">**Workaround**</span></span>  
> <span data-ttu-id="a27a6-402">와 같은 매개 변수의 데이터 형식을 명시적으로 설정할 `SqlDbType` 또는 `DbType`합니다.</span><span class="sxs-lookup"><span data-stu-id="a27a6-402">Explicitly set the data type for parameters such as `SqlDbType` or `DbType`.</span></span> <span data-ttu-id="a27a6-403">이 BLOB 데이터 형식의 경우 중요 (`image` 고 `ntext`).</span><span class="sxs-lookup"><span data-stu-id="a27a6-403">This is critical in the case of BLOB data types (`image` and `ntext`).</span></span> <span data-ttu-id="a27a6-404">다음과 같은 코드를 사용 합니다.</span><span class="sxs-lookup"><span data-stu-id="a27a6-404">Use code like the following:</span></span>
> 
...
## <a name="for-more-information"></a><span data-ttu-id="a27a6-405">자세한 내용</span><span class="sxs-lookup"><span data-stu-id="a27a6-405">For More Information</span></span>

<span data-ttu-id="a27a6-406">WebMatrix 베타 3에 대 한 자세한 내용은 다음 웹 사이트를 참조 하세요.</span><span class="sxs-lookup"><span data-stu-id="a27a6-406">For more information about WebMatrix Beta 3, see the following websites:</span></span>

- [<span data-ttu-id="a27a6-407">IIS.net</span><span class="sxs-lookup"><span data-stu-id="a27a6-407">IIS.net</span></span>](http://iis.net/)
- [<span data-ttu-id="a27a6-408">ASP.NET</span><span class="sxs-lookup"><span data-stu-id="a27a6-408">ASP.NET</span></span>](https://asp.net/webmatrix)
- [<span data-ttu-id="a27a6-409">Microsoft.com/web</span><span class="sxs-lookup"><span data-stu-id="a27a6-409">Microsoft.com/web</span></span>](https://www.microsoft.com/web)

---

<span data-ttu-id="a27a6-410">© 2010 Microsoft Corporation.</span><span class="sxs-lookup"><span data-stu-id="a27a6-410">© 2010 Microsoft Corporation.</span></span> <span data-ttu-id="a27a6-411">All Rights Reserved.</span><span class="sxs-lookup"><span data-stu-id="a27a6-411">All Rights Reserved.</span></span> <span data-ttu-id="a27a6-412">[사용 약관](https://msdn.microsoft.cos/cc300389.aspx)합니다.</span><span class="sxs-lookup"><span data-stu-id="a27a6-412">[Terms of Use](https://msdn.microsoft.cos/cc300389.aspx).</span></span>
