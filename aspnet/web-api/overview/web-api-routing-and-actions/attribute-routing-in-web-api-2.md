...
규칙 기반 라우팅의 장점 중 하나는 한 곳에 정의 된 템플릿 및 라우팅 규칙은 모든 컨트롤러에서 일관 되 게 적용 됩니다. 그러나 규칙 기반 라우팅 하기가 어렵습니다 RESTful Api에 공통 되는 특정 URI 패턴을 지원 하도록 합니다. 예를 들어, 리소스는 종종 자식 리소스를 포함 합니다. 고객이 주문을 갖고, 영화 행위자가 books authors 있고 등. 이러한 관계를 반영 하는 Uri를 만드는 자연 스러운 것:
...
| 제약 조건 | 설명 | 예제 |
| --- | --- | --- |
| 알파 | 일치 대문자 또는 소문자로 라틴어 알파벳 문자 (a-z A-z) | {x:alpha} |
