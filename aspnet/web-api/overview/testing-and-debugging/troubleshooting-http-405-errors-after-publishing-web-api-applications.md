...
> 이 자습서에서는 웹 API 응용 프로그램을 프로덕션 웹 서버에 게시 한 후 HTTP 405 오류 문제를 해결 하는 방법을 설명 합니다.
> 
...
웹 API 응용 프로그램은 일반적으로 다음과 같은 몇 가지 일반적인 HTTP 동사를 사용 합니다. GET, POST, PUT, DELETE 및 경우에 따라 패치를 적용 합니다. 즉, 개발자는 해당 동사가 프로덕션 서버의 다른 IIS 모듈에 의해 구현 되는 상황이 발생할 수 있습니다. 그러면 Visual Studio 또는 개발 서버에서 올바르게 작동 하는 Web API 컨트롤러가 다음을 반환 합니다. 프로덕션 서버에 배포 하는 경우 HTTP 405 오류가 발생 합니다. 다행히이 문제는 쉽게 해결 되지만 해결 방법에 문제가 발생 하는 이유에 대 한 설명이 있습니다.

## <a name="what-causes-http-405-errors"></a>HTTP 405 오류를 발생 시키는 원인

HTTP 405 오류 문제를 해결 하는 방법에 대 한 첫 번째 단계는 HTTP 405 오류가 실제로 무엇을 의미 하는지 이해 하는 것입니다. Http에 대 한 기본 관리 문서는 [RFC 2616](http://www.ietf.org/rfc/rfc2616.txt)입니다 .이는 http 405 상태 코드를 ***허용 되지 않는 메서드로***정의 하 고, 요청 줄에 지정 된 &quot;메서드가 허용 되지 않는 상황으로이 상태 코드를 추가로 설명 합니다. 요청 URI로 식별 되는 리소스입니다.&quot; 즉, http 동사는 HTTP 클라이언트가 요청한 특정 URL에 허용 되지 않습니다.
...
다음 예제 HTTP 요청 및 응답은 HTTP 클라이언트가 웹 서버에서 Web API 응용 프로그램에 값을 삽입 하려고 시도 하는 상황을 보여 줍니다. 서버는 PUT 메서드가 허용 되지 않는다는 내용의 HTTP 오류를 반환 합니다.
...
특정 HTTP 동사가 허용 되지 않는 이유는 여러 가지가 있지만 IIS에서이 오류가 발생 하는 가장 큰 원인은 다음과 같습니다. 여러 처리기가 동일한 동사/메서드에 대해 정의 되 고 처리기 중 하나가 필요한 처리기를 차단 하 고 있습니다. 요청을 처리 하 고 있습니다. 설명 방식으로 IIS는 Applicationhost.config 및 web.config 파일의 주문 처리기 항목을 기반으로 처음부터 마지막으로 처리기를 처리 합니다. 여기서 경로, 동사, 리소스 등의 첫 번째 일치 하는 조합이 요청을 처리 하는 데 사용 됩니다.
...
위의 예제에서 WebDAV 처리기와 ASP.NET (Web API에 사용 됨)에 대 한 확장 URL 처리기는 별도의 HTTP 메서드 목록에 대해 명확 하 게 정의 됩니다. 이 구성으로 인해 오류가 발생 하지는 않지만 ISAPI DLL 처리기가 모든 HTTP 메서드에 대해 구성 되어 있습니다. 그러나 HTTP 405 오류 문제를 해결할 때 이와 같은 구성 설정을 고려해 야 합니다.
...
이 시나리오는 응용 프로그램이 개발 환경에서 프로덕션 환경으로 게시 된 후에 발생 하며,이는 처리기/모듈의 목록이 개발 환경과 프로덕션 환경 간에 다르기 때문에 발생 합니다. 예를 들어 Visual Studio 2012 이상 버전을 사용 하 여 Web API 응용 프로그램을 개발 하는 경우 IIS Express는 테스트를 위한 기본 웹 서버입니다. 이 개발 웹 서버는 서버 제품에서 제공 되는 전체 IIS 기능을 축소 한 버전으로,이 개발 웹 서버에는 개발 시나리오에 추가 된 몇 가지 변경 내용이 포함 되어 있습니다. 예를 들어, 일반적으로 사용 되지 않는 IIS의 전체 버전을 실행 하는 프로덕션 웹 서버에 WebDAV 모듈이 설치 되어 있는 경우가 있습니다. IIS의 개발 버전 (IIS Express)은 WebDAV 모듈을 설치 하지만 WebDAV 모듈의 항목은 의도적으로 주석 처리 되어 있으므로 IIS Express 구성을 특별히 변경 하지 않는 한 WebDAV 모듈이 IIS Express에 로드 되지 않습니다. IIS Express 설치에 WebDAV 기능을 추가 하는 설정입니다. 따라서 웹 응용 프로그램은 개발 컴퓨터에서 제대로 작동할 수 있지만 웹 API 응용 프로그램을 프로덕션 웹 서버에 게시할 때 HTTP 405 오류가 발생할 수 있습니다.
...
HTTP 501 오류 메시지를 수신 하는 상황이 발생 합니다. 즉, 특정 기능이 서버에 구현 되지 않은 경우에는 IIS 설정에 정의 된 처리기가 HTTP 요청과 일치 하는 것을 의미 하는 경우가 많습니다. 는 시스템에 항목이 제대로 설치 되지 않았거나 특정 HTTP 메서드를 지 원하는 처리기가 정의 되지 않았음을 나타낼 수 있습니다. 이 문제를 해결 하려면 해당 모듈 또는 처리기 정의가 없는 HTTP 메서드를 사용 하려는 모든 응용 프로그램을 다시 설치 해야 합니다.
