...
4. 이 Azure 웹 사이트에 게시 한 경우 이전에 체크 인할 수는 **대상에서 추가 파일 제거** 응용 프로그램 설정을 완전히 게시 합니다. 알림 합니다 **Windows Azure 인증 사용** 확인란을 선택 합니다.

    ![](windows-azure-authentication/_static/image10.png)
5. 선택 사항: 에 **미리 보기** 탭을 클릭 **미리 보기 시작** 배포 파일을 볼 수 있습니다.
...
8. 응용 프로그램이 성공적으로 게시 되 면 게시 된 웹 사이트는 브라우저에서 열립니다.
    ...
    > [!NOTE]
    > 최대 5 분 (일반적으로 반드시 적은) 완전히 프로 비전 할 Windows Azure Active Directory를 사용 하 여 대상 호스트에 대 한 Windows Azure 인증을 사용 하도록 설정한 후 응용 프로그램에 대 한 걸릴 수 있습니다. 처음 실행할 때 응용 프로그램 ACS50001 오류가 표시 되 면: 이름 '[realm]'를 사용 하 여 신뢰 당사자를 찾을 수 없습니다 몇 분 정도 기다렸다가 고 응용 프로그램을 다시 실행 하십시오.
...
#### <a name="role-based-authorization-fails-when-using-windows-azure-authentication"></a>Windows Azure 인증을 사용 하는 경우 역할 기반 권한 부여 실패

역할 기반 권한 부여를 수행할 수 있도록 Windows Azure 인증 필요한 역할 클레임을 현재 제공 하지 않습니다. Windows Azure Active Directory에서 인증된 된 사용자의 역할을 수동으로 검색 해야 합니다.

#### <a name="browsing-to-an-application-with-windows-azure-authentication-results-in-the-error-acs20016-the-domain-of-the-logged-in-user-livecom-does-not-match-any-allowed-domain-of-this-sts"></a>Azure 인증 결과 오류가 "ACS20016 도메인에서 로그인된 한 사용자 (live.com)의 모든 일치 하지 않습니다. 허용 되는 도메인의 STS" windows 응용 프로그램으로 이동

때문에 400 오류 응답 (예: hotmail.com, live.com, outlook.com) Microsoft 계정에 이미 로그인 하 고 Windows Azure 인증을 설정한 응용 프로그램에 액세스 하려고 하는 경우 발생할 수 있습니다 Microsoft 계정 도메인 Windows Azure Active Directory에서 인식 되지 않습니다. 응용 프로그램에 로그인, 로그 아웃 Microsoft 계정에서 먼저 합니다.

#### <a name="logging-into-an-application-with-windows-azure-authentication-enabled-and-a-x509certificatevalidationmode-other-than-none-results-in-certificate-validation-errors-for-the-accountsaccesscontrolwindowsnet-certificate"></a>Accounts.accesscontrol.windows.net 인증서에 대 한 인증서 유효성 검사 오류가 없음 결과 이외의 다른 Windows Azure 인증을 사용 하도록 설정 하 고는 X509CertificateValidationMode를 사용 하 여 응용 프로그램에 로그인

인증서 유효성 검사는 필요 하지 않습니다 하 고 해제 해야 합니다. 발급자 인증서의 지문은 WSFederationAuthenticationModule에서 유효성이 검사 됩니다.

#### <a name="when-attempting-to-enable-windows-azure-authentication-the-web-authentication-dialog-shows-the-error-acs20016-the-domain-of-the-logged-in-user-contosoonmicrosoftcom-does-not-match-any-allowed-domain-of-this-sts"></a>Windows Azure 인증을 사용 하려고 할 때 웹 인증 대화 상자를 표시 합니다 "ACS20016: 로그인된 한 사용자 (contoso.onmicrosoft.com)의 도메인 일치 하지 않습니다이 STS의 허용 된 모든 도메인. "

이전에 성공적으로 로그인 하면 동일한 Visual Studio 프로세스 내에서 다른 Windows Azure Active Directory 계정을 사용 하는 경우이 오류가 표시 될 수 있습니다. 지정한 계정에서 로그 아웃 하거나 Visual Studio를 다시 시작 합니다. 이전에 로그인 하 고 "로그인 유지" 옵션을 선택한 경우 브라우저 쿠키를 지우려면 해야 할 수 있습니다.
...
Azure 서비스 중 하나에 다른 Microsoft ID로 이미 로그인 되어 발생할 수 있습니다. 사용 하 여 개인 브라우저 창 IE에서 InPrivate 또는 Incognito Chrome에서 같은 또는 모든 쿠키의 선택을 취소 합니다.
