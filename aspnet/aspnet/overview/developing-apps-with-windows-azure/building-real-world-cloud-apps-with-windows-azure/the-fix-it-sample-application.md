...
9. 기본 (큐 처리)에 대 한 웹 앱을 만드는 앱 문제 해결 환경 생성 스크립트를 실행 합니다.
    ...
    스크립트 실패 하거나 같은 오류를 생성 하는 경우 "New-azurewebsite: Set-azuresubscription 및 Select-azuresubscription를 먼저 호출"있습니다 하지 완료 했을 수의 Azure PowerShell을 구성 합니다.
...
- `Get-AzureSqlDatabase`: 이 cmdlet을 실행 하려면 데이터베이스 서버 이름을 파이프할 `Get-AzureSqlDatabase`:   `Get-AzureSqlDatabaseServer | Get-AzureSqlDatabase.`
