...
구성 요소는 일반적으로 구현 됩니다 *.cshtml* 조합 하 여 파일을 C# 와 HTML 태그입니다. 구성 요소에 대 한 UI는 HTML을 사용 하 여 정의 됩니다. 동적 렌더링 논리(예: 루프, 조건, 식)는 [Razor](xref:mvc/views/razor)라는 포함된 C# 구문을 사용하여 추가됩니다. 구성 요소 Razor 앱을 컴파일할 때, HTML 태그 및 C# 렌더링 논리 구성 요소 클래스 변환 됩니다. 파일의 이름과 일치 하는 생성된 된 클래스의 이름입니다.

에 정의 된 구성 요소 클래스의 멤버는 `@functions` 블록 (둘 이상의 `@functions` 블록은 허용). 에 `@functions` 이벤트 처리를 위해 또는 다른 구성 요소 논리를 정의 하기 위한 메서드와 함께 지정 된 블록에 구성 요소 상태 (속성, 필드).
...
기본 클래스에서 파생 되어야 `BlazorComponent`합니다.
...
| [\@inherits](xref:mvc/views/razor#section-3) | 구성 요소가 상속 된 클래스의 모든 권한을 제공 합니다. |
| [\@inject](xref:mvc/views/razor#section-4) | 서비스에서 주입 수 있도록 합니다 [서비스 컨테이너](xref:fundamentals/dependency-injection)합니다. 자세한 내용은 [보기에 종속성 주입](xref:mvc/views/dependency-injection)을 참조하세요. |
