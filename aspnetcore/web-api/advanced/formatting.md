...
![Fiddler 콘솔: 요청에 대 한 Raw 탭에 Accept 헤더 값은 application/xml을 보여 줍니다. 응답에 대한 Raw 탭에 application/xml의 Content-Type 헤더 값이 표시됩니다.](formatting/_static/xml-response.png)
...
![Fiddler 콘솔: 요청에 대 한 Raw 탭에 Accept 헤더 값은 application/json을 보여 줍니다. 응답에 대한 Raw 탭에 application/json의 Content-Type 헤더 값이 표시됩니다.](formatting/_static/json-response-fiddler.png)
...
클라이언트는 쿼리 문자열 또는 일부 경로와 같은 URL의 일부로, 또는 .xml이나 .json과 같은 형식별 파일 확장명을 사용하여 특정 형식을 요청할 수 있습니다. 요청 경로의 매핑은 API가 사용하는 경로에 지정해야 합니다. 예를 들어:
