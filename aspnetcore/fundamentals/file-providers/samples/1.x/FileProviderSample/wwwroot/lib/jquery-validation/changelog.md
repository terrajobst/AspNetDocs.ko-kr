...
## <a name="additional"></a>추가
  * cifES 및 nieES 알고리즘을 구체화합니다. #1826을 마감합니다.

## <a name="build"></a>빌드
  * NPM 패키지에 축소된 버전 포함
  * dev-dependencies를 최신 버전으로 범프

## <a name="core"></a>코어
  * 단추 종류를 사용하여 입력에 대한 바인딩을 추가합니다. #1891을 마감합니다.
  * jquery3을 지원합니다. #1866을 마감합니다.
  * jQuery 별칭 ‘expr[“:”]’을 ‘expr.pseudos’로 변경

## <a name="localisation"></a>지역화
  * 우르두어 번역을 추가합니다. #1873을 마감합니다.

## <a name="localization"></a>지역화
  * az 번역의 잘못된 파일 확장명을 수정했습니다. #1890을 마감합니다.
  * pt-BR에 누락된 번역 추가(#1897 마감)
  * 아라비아 언어 파일의 오타를 수정했습니다.

## <a name="tests"></a>테스트
  * QUnit을 2.0으로 업그레이드합니다.

## <a name="umd"></a>UMD
  * CommonJS에 대한 지원이 향상되었습니다.

<a name="1151--2016-07-22"></a>1.15.1 / 2016-07-22
==================

## <a name="additional"></a>추가
  * 여러 MIME 형식 유효성 검사 수정
  * IBAN에는 5자 이상이 필요함(#1797 마감, #1674 수정)
  * notEqualTo jQuery 참조 수정

## <a name="core"></a>코어
  * #1805에 대한 실패 테스트를 추가함
  * 3개 이상의 필드를 사용하여 그룹 유효성 검사 수정
  * #1644 및 #1657에 도입된 재발 수정(#1800 마감)
  * 부동 소수점 숫자를 올바르게 처리하기 위한 단계 유효성 검사 업데이트
  * 양식이 아닌 요소에서 $.fn.rules() 호출 시 오류 수정
  * 유효성 검사기 범위에서 `errorPlacement` 호출
  * 단일 입력 유효성 검사에 대한 이벤트에서 예외가 발생하는 양식에서 contenteditable 요소 관련 문제를 해결함

## <a name="demo"></a>데모
  * 부트스트랩 및 Semantic-UI 데모에 대한 링크를 index.html에 추가
  * `.validateDelegate()` 대신 `.on()` 사용

## <a name="localization"></a>지역화
  * 아제리어를 추가함

## <a name="tests"></a>테스트
  * PR #1760에 대한 재발 단위 테스트를 추가함
...
## <a name="core"></a>코어
  * `resetForm`은 요소에서 `valid` 클래스를 제거해야 합니다.
  * 원격 규칙을 사용할 때 이미 강조된 경우 필드의 강조를 취소합니다.
  * `equalTo` 규칙에 `blur` 이벤트를 한 번만 바인드
  * 빈 jquery 집합에서 .rules()를 호출할 때 발생하는 오류를 해결했습니다.
  * 입력 그룹의 오류 메시지 처리를 수정합니다.
  * `groups` 설정을 사용할 때 `showLabel`의 TypeError 수정
  * 메서드 이름을 원격에 전달하는 방법 추가
  * 다음 필드가 이미 채워져 있을 때 유효성 검사를 트리거하지 못함(#1508 수정)
  * 필수 규칙이 숫자 및 자릿수 규칙보다 우선
  * 오류를 숨겼지만 입력 오류 클래스는 제거하지 않음
  * 원격 유효성 검사에서 잘못된 오류 메시지를 사용함
  * 원격 유효성 검사의 필드 강조를 수정했습니다.
  * 다중 선택 요소에 대한 `:filled` 선택기를 수정했습니다.
  * jQuery.validator.methods에 문서 참조를 추가함
  * `formatAndAdd`에서 `defaultMessage`로 메시지 처리 이동
  * ErrorList에 포함해야 하는 오류만 포함
  * "C:\fakepath\"를 포함하지 않고 파일 이름 추출
  * HTML5 단계 특성 지원. #1295 수정
  * 대기 중인 요청에서 “보류 중” 클래스에 대한 지원이 추가됨
  * 노멀라이저가 추가됨(#1602)
  * `creditcard` 메서드 분할
  * aria-describedby를 빌드하지 않는 regex에서 사용할 errorID 이스케이프
  * Sizzle 오류가 throw되지 않도록 이름에서 작은따옴표를 이스케이프함
  * API 호출을 건너뛰기 위해 유효성 검사를 하는 필드의 값을 사용하는 대신, 요청의 직렬화된 데이터 개체 사용
  * contentEditable 태그에 대한 지원 추가

## <a name="additional"></a>추가
  * BIC: 위치의 두 번째 위치에서 숫자 1-9 허용
  * accept 메서드 regex를 올바르게 이스케이프해야 합니다.
  * BIC에 대해 대/소문자 구분하지 않는지 검사
  * 잘못된 조합을 제외하도록 postalCodeCA 수정
  * postalCodeCA 메서드를 더 느슨하게 만듦

## <a name="localization"></a>지역화
  * 마케도니아어 지역화가 추가되었습니다.
  * 폴란드어(adamwojtkiewicz)에 누락된 패턴 메시지가 추가됨
  * 최소/최대 메시지의 페르시아어 번역을 수정했습니다.
  * messages_sk.js를 업데이트함
  * 말레이어 번역을 업데이트함
  * 추가 메서드의 메시지를 포함함
  * pt_BR 번역 개선 및 ‘cifES’ 키의 오타 수정
