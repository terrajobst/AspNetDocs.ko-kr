...
| 세션 | 데이터 보호(암호화된 쿠키)(<xref:security/data-protection/configuration/overview> 참조) 및 캐싱(<xref:performance/caching/distributed> 참조).<br><br>자세한 내용은 참조 하세요. [세션 및 앱 상태: 세션 상태](xref:fundamentals/app-state#session-state)합니다. |
| TempData | 데이터 보호 (암호화 된 쿠키) (참조 <xref:security/data-protection/configuration/overview>) 또는 세션 (참조 [세션 및 앱 상태: 세션 상태](xref:fundamentals/app-state#session-state)).<br><br>자세한 내용은 참조 하세요. [세션 및 앱 상태: TempData](xref:fundamentals/app-state#tempdata). |
